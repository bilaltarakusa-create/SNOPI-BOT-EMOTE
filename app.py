# Imports - تم إزالة fastapi, uvicorn, pydantic وإضافة flask
import requests , os , psutil , sys , jwt , pickle , json , binascii , time , urllib3 , base64 , datetime , re , socket , threading , ssl , pytz , aiohttp
from flask import Flask, request, jsonify # <-- تم الإضافة
import asyncio # للتأكد من وجوده
...
from threading import Thread # التأكد من استيراد Thread
...
def get_random_color():
    colors = [
        "[FF0000]", "[00FF00]", "[0000FF]", "[FFFF00]", "[FF00FF]", "[00FFFF]", "[FFFFFF]", "[FFA500]","[A52A2A]", "[800080]", "[000000]", "[808080]", "[C0C0C0]", "[FFC0CB]", "[FFD700]", "[ADD8E6]",
        "[90EE90]", "[D2691E]", "[DC143C]", 
        "[00CED1]", "[9400D3]", "[F08080]", "[20B2AA]", "[FF1493]",
        "[7CFC00]", "[B22222]", "[FF4500]", "[DAA520]", "[00BFFF]", "[00FF7F]", "[4682B4]", "[6495ED]", "[5F9EA0]", "[DDA0DD]", "[E6E6FA]", "[B0C4DE]", "[556B2F]", "[8FBC8F]", "[2E8B57]", "[3CB371]",
...
async def GeNeRaTeAccEss(uid , password):
    ...
    data = {
        "uid": 4203825982,
        "password": "0A8603BF44DB577CC4D5445BD0CA0B690E154F3570935E165190788F43A957AA",
        "response_type": "token",
        "client_type": "2",
...
class CLIENT:
    def __init__(self):
        ...
        self.data = None
        self.data2 = None
        self.key = None # سيتم تخزين المفتاح هنا
        self.iv = None # سيتم تخزين IV هنا
        self.STATUS = None
        self.AutHToKen = None
        self.OnLineiP = None
        ...
        self.JWT = None # تمت إضافته لتخزين التوكن
        self.response = None
        self.uid = None
        self.chat_id = None
        self.XX = None
    ...
    async def SEndPacKeT(self,ChaT , OnLineE , TypE , PacKeT): # تم تعديل الترتيب ليطابق الاستدعاءات
        if TypE == 'ChaT' and ChaT: self.whisper_writer.write(PacKeT) ; await self.whisper_writer.drain()
        elif TypE == 'OnLine' and OnLineE: self.online_writer.write(PacKeT) ; await self.online_writer.drain() # التأكد من أن OnLineE (self.online_writer) موجود
        else: 
            if TypE == 'OnLine' and not OnLineE: print("Error: online_writer is None.")
            elif TypE == 'ChaT' and not ChaT: print("Error: whisper_writer is None.")
                    ...
                    if self.data2:
                        ...
                        if self.data2.hex().startswith('0500') and len(self.data2.hex()) > 1000 and joining_team:
                            try:
                                ...
                                Message = f"[B][C]{get_random_color()}\n- مرحبا بك في BOT SNOPI ! \n\n{get_random_color()}- Commands : @a {xMsGFixinG('12345678')} {xMsGFixinG('909000001')}\n\n[00FF00]Dev : @{xMsGFixinG('SNOPI-HEX-EMOT')}"
                    ...
                    if data.hex().startswith("120000"):
                        ...
                        if self.response:
                            ...
                            if self.inPuTMsG.startswith('/x/'):
                                # *** تم تصحيح الخطأ هنا ***
                                CodE = self.inPuTMsG.split('/x/')[1] # كان الخطأ 1K
                                print(CodE)
                                try:
                                    dd = chatdata['5']['data']['16']
                                    print('msg in private')
                            ...
                            if self.inPuTMsG in ("hi" , "hello" , "fin" , "fen" , "wach", "slm", "cc", "salam"):
                                uid = self.response.Data.uid
                                chat_id = self.response.Data.Chat_ID
                                message = 'Hello Im Dev fakhm\nTelegram : @fakhm_bott213'
                                P = await self.SEndMsG(self.response.Data.chat_type , message , uid , chat_id , key , iv)
                                await self.SEndPacKeT(self.whisper_writer , self.online_writer , 'ChaT' , P)
    ...
    async def MaiiiinE(self):
        Uid , Pw = '4203825982', "0A8603BF44DB577CC4D5445BD0CA0B690E154F3570935E165190788F43A957AA",
        ...
        self.key = MajoRLoGinauTh.key # تخزين المفتاح
        self.iv = MajoRLoGinauTh.iv # تخزين IV
...
# =============================================================================
#  الكود الجديد الخاص بـ API (تم التحويل إلى Flask)
# =============================================================================

# 1. إنشاء تطبيق Flask
app = Flask(__name__)

# 2. إنشاء نسخة من الكلاينت الخاص بك (نفس مكان التعريف الأصلي)
client = CLIENT()

# 3. متغيرات عامة لربط Flask بـ asyncio
bot_loop = None # سيحتفظ بـ event loop الخاص بالبوت

def run_async_bot():
    """
    تعمل هذه الدالة في thread منفصل لتشغيل
    الـ event loop الخاص بـ asyncio والبوت.
    """
    global bot_loop
    try:
        bot_loop = asyncio.new_event_loop()
        asyncio.set_event_loop(bot_loop)
        print("--- يتم الآن تشغيل البوت في الخلفية... ---")
        bot_loop.run_until_complete(StarTinG())
        print("--- توقف البوت عن العمل ---")
    except Exception as e:
        print(f"--- [خطأ] انهار الـ thread الخاص بالبوت: {e} ---")
        import traceback
        traceback.print_exc()

# =============================================================================
#  *** 3. (مساعد) - تم تعديل الدالة لتقبل قائمة ***
# =============================================================================
async def _send_emote_async(uids_list, emote_id):
    """
    دالة مساعدة (coroutine) لإرسال الـ emote
    سيتم استدعاؤها من داخل route الـ Flask.
    *** تقبل الآن قائمة من الـ UIDs. ***
    """
    global client
    
    # التأكد من أن البوت متصل وجاهز
    if not client.online_writer or not client.key or not client.iv:
        return (False, "البوت غير متصل أو غير جاهز بعد. يرجى المحاولة بعد لحظات.")
    
    success_uids = []
    failed_uids = []
    
    try:
        # المرور على كل UID في القائمة
        for uid in uids_list:
            try:
                print(f"Async Task: Sending emote {emote_id} to {uid}")
                
                # إنشاء حزمة الـ Emote
                emote_packet = await Emote_k(uid, emote_id, client.key, client.iv)
                
                # إرسال الحزمة عبر اتصال 'OnLine'
                await client.SEndPacKeT(None, client.online_writer, 'OnLine', emote_packet)
                
                # إضافة تأخير بسيط جداً بين الطلبات لتجنب إغراق السيرفر
                await asyncio.sleep(0.05) # 50ms delay
                
                success_uids.append(uid)
                
            except Exception as e:
                print(f"Failed to send emote to UID {uid}: {e}")
                failed_uids.append(uid)
        
        # إعداد رسالة النتيجة
        message = f"Emote {emote_id} sent. Success: {len(success_uids)}. Failed: {len(failed_uids)}."
        if success_uids:
            message += f" Success UIDs: {success_uids}."
        if failed_uids:
            message += f" Failed UIDs: {failed_uids}."
            
        return (True, message)
    
    except NameError as ne:
        print(f"خطأ: الدالة 'Emote_k' غير معرفة. تأكد من وجودها في xPARA.py. {ne}")
        return (False, "خطأ في الخادم: الدالة 'Emote_k' غير موجودة.")
    except Exception as e:
        print(f"حدث خطأ أثناء إرسال الـ emote: {e}")
        import traceback
        traceback.print_exc()
        return (False, f"حدث خطأ: {e}")

# =============================================================================
#  *** 4. إنشاء نقطة النهاية (Endpoint) - تم التعديل ***
# =============================================================================
@app.route("/emote/<string:uids>/<int:emote_id>", methods=["GET"])
def api_send_emote(uids, emote_id):
    """
    هذا هو الـ API route. يعمل في thread الـ Flask (متزامن).
    يستقبل الـ emote_id ورابطة من الـ UIDs (مفصولة بفاصلة)
    
    مثال للاستدعاء (لاعب واحد):
    GET http://127.0.0.1:8000/emote/12345678/90900001
    
    مثال للاستدعاء (3 لاعبين):
    GET http://127.0.0.1:8000/emote/123,456,789/90900001
    """
    global bot_loop
    
    # تحويل سلسلة الـ UIDs إلى قائمة من الأرقام
    try:
        # 1. تقسيم السلسلة بالفاصلة
        uid_strings = uids.split(',')
        # 2. تحويل كل جزء إلى رقم صحيح (int)
        uids_list = [int(uid_str.strip()) for uid_str in uid_strings] 
        
        if not uids_list:
            raise ValueError("No UIDs provided.")
            
    except (ValueError, TypeError):
        return jsonify({"error": "Invalid 'uids' format. Must be a single ID or multiple IDs separated by commas (e.g., 123,456)."}), 400
    
    # التأكد من أن البوت يعمل
    if not bot_loop or not bot_loop.is_running():
        return jsonify({"error": "Bot is not running or event loop is not available."}), 503

    # جدولة الدالة الـ async لتعمل على الـ event loop الخاص بالبوت
    future = asyncio.run_coroutine_threadsafe(
        _send_emote_async(uids_list, emote_id), # إرسال القائمة
        bot_loop
    )
    
    try:
        # انتظار النتيجة (هذا يوقف thread الـ Flask مؤقتًا)
        # تمت زيادة الوقت الأقصى ليناسب إرسال عدة رقصات
        timeout_duration = max(10, len(uids_list) * 2) # ثانيتين لكل لاعب، بحد أدنى 10 ثواني
        success, message = future.result(timeout=timeout_duration) 
        ...
        return jsonify({"error": "Request timed out while communicating with the bot."}), 504
    except Exception as e:
